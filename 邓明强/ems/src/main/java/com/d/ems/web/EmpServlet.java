...
import com.d.ems.service.impl.EmpService;
...
public class EmpServlet extends HttpServlet {
	 ...
	 */
	private void lregister(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		// 首先获取到前台页面传递的数据
		...
		if (flag == 1) {
			...
			// 请求转发
			req.getRequestDispatcher("/lregister.jsp").forward(req, resp);
			...
			// 调用注册的方法
			service.lregisterEmp(emp);

			resp.getWriter().write("注册成功, 3s后跳转到登录页面");

			resp.setHeader("refresh", "3;url=/ems/login2.jsp");
		}
